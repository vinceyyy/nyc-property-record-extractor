...
def query(api, args=None):
    headers = {
        "X-App-Token": "S90v4Qd6Lkq958QXemPDPyYuK",
    }
    if args is not None:
...
def query_sup(api, master_df):
    df = pd.DataFrame()
    # by years
    start = master_df.sort_values('document_date').head(1).reset_index(
        drop=True).loc[0, 'document_date'][:4]
    end = master_df.sort_values('document_date').tail(1).reset_index(
        drop=True).loc[0, 'document_date'][:4]
    years = range(int(start), int(end) + 1)
    for year in years:
        df = df.append(pd.DataFrame(
            query(api, f"$where=starts_with(document_id, '{year}')")), ignore_index=True)
    # non regular id
    sup_df = master_df.loc[~master_df['document_id'].str[:4].isin(
        str(year) for year in years)]
    id_list = sup_df['document_id'].to_list()
    for id in id_list:
        sup = pd.DataFrame(
            query(api, f"document_id='{id}'"))
...
# %%
master_raw = pd.DataFrame(query(
    acris_master, f"$where=document_date between '{start}-01-01T00:00:00' and '{end}-12-31T23:59:59' AND doc_type in('DEED', 'RPTT%26RET')"))
master = master_raw[['document_id', 'document_date', 'document_amt']]

# %%
legal_raw = query_sup(acris_legal, master)
legal = legal_raw[['document_id', 'borough', 'block', 'lot',
                   'property_type', 'street_number', 'street_name', 'unit']]
parties_raw = query_sup(acris_parties, master)
parties = parties_raw[parties_raw['party_type']
                      == '2'][['document_id', 'name']]
